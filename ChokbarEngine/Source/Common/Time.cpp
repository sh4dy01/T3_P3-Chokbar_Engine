...
{
	time_t now = time(NULL);		// Get current time in UNIX format
	tm ltm;							// Create tm struct that will be used for converting to local time
	localtime_s(&ltm, &now);		// Convert UNIX time to local time
